...
if (isset($_GET['id'])) {
    $post_id = (int) ($_GET['id']);
    $sql = "SELECT posts.*, users.first_name as seller_name FROM posts JOIN users ON posts.user_id = users.id";
...
<style>
    ...
    @media (max-width: 991px) {
        .d-grid > div:nth-child(1) { order: 1; }
        .d-grid > div:nth-child(2) { order: 2; }
        .d-grid > div:nth-child(3) { order: 3; }
    }
    @media (min-width: 992px) {
        .d-grid { grid-template-columns: 8fr 4fr !important; }
        .d-grid > div:nth-child(1) { grid-column: 1 !important; grid-row: 1 !important; }
        .d-grid > div:nth-child(2) { grid-column: 2 !important; grid-row: 1 / span 2 !important; align-self: start !important; }
        .d-grid > div:nth-child(3) { grid-column: 1 !important; grid-row: 2 !important; }
    }
</style>
...
<body class="bg-light">
    ...
    <div class="container py-4">
        <div class="d-grid gap-4"
            style="grid-template-columns: 1fr; @media (min-width: 992px) { grid-template-columns: 8fr 4fr; }">

            <div style="grid-order: 1; @media (min-width: 992px) { grid-column: 1; grid-row: 1; }">
            ...
            <div
                style="grid-order: 2; @media (min-width: 992px) { grid-column: 2; grid-row: 1 / span 2; align-self: start; }">
            ...
            <div style="grid-order: 3; @media (min-width: 992px) { grid-column: 1; grid-row: 2; }">
                ...
                <div class="modal-content border-0 shadow-lg">
                    <div class="modal-header bg-light">
                        <h5 class="modal-title fw-bold fs-6 text-dark" id="contactSellerModalLabel">
                            ...
                            Nouvelle discussion avec
                            <?= htmlspecialchars($post['seller_name'] ?? 'le vendeur', ENT_QUOTES, 'UTF-8') ?>
                        </h5>
                    ...
                    <form action="/handlers/chat-handler.php" method="POST">
                        ...
                        <div class="modal-body p-4">
                            <div class="p-3 bg-light rounded mb-3 border">
                                <small class="text-muted d-block text-uppercase fw-semibold" style="font-size: 0.7rem;">À
                                    propos de l'annonce</small>
                                <span
                                    class="fw-bold text-dark d-block text-truncate"><?= htmlspecialchars($post['title'], ENT_QUOTES, 'UTF-8') ?></span>
                                <span class="fw-bold text-danger"><?= number_format($post['price'], 0, ',', ' ') ?> €</span>
                            </div>

                            <div class="mb-3">
                                <label for="first_message" class="form-label fw-bold small text-secondary">Votre premier
                                    message</label>
